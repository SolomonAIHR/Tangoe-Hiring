export default async function handler(req, res) {
  ...
  // ── SAVE NEW JD ──────────────────────────────────────────────────────────
  try {
    const { role, req: reqNum, jd, questions, focus, level, camera, pasteDetect,
            tabDetect, photoInterval, maxUses, expiryDays, expiresAt } = body;

    if (!jd || !role) return res.status(400).json({ error: 'Missing role or JD' });

    // Generate short ID
    const id = Math.random().toString(36).substring(2, 10);

    const config = {
      role, req: reqNum || '', jd, questions: parseInt(questions) || 6,
      focus: focus || 'balanced', level: level || 'mid',
      camera: camera !== false, pasteDetect: pasteDetect !== false,
      tabDetect: tabDetect !== false, photoInterval: parseInt(photoInterval) || 60,
    ...
    const d = await r.json();
    if (d.result !== 'OK') return res.status(500).json({ error: 'Failed to save', detail: d });
