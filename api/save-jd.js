export default async function handler(req, res) {
  ...
  try {
    const { role, req: reqNum, jd, questions, focus, level, camera,
            pasteDetect, tabDetect, photoInterval, maxUses, expiryHours, expiresAt } = body;
    ...
    const config = {
      ...
      expiryHours: parseInt(expiryHours) || 48,
    ...
    const saveRes = await fetch(`${KV_URL}/set/jd:${id}`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${KV_TOKEN}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ value: JSON.stringify(config), ex: 7776000 })
    });
    ...
    if (saveData.result !== 'OK') {
      return res.status(500).json({ error: 'Failed to save to Upstash', detail: saveData });
    }

    // Update index — safely handle any format
    let ids = [];
    try {
      ...
      if (idxData.result) {
        const parsed = JSON.parse(idxData.result);
        ids = Array.isArray(parsed) ? parsed : [];
      }
    } catch(e) {
      ids = []; // start fresh if index is corrupted
    }

    ids = [id, ...ids.filter(x => x && x !== id)].slice(0, 200);
    ...
    return res.status(500).json({ error: error.message, stack: error.stack?.substring(0,200) });
  }
}
