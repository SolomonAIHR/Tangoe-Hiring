export default async function handler(req, res) {
  ...
  try {
    ...
    // Save individual result — keep for 90 days
    await fetch(`${KV_URL}/set/result:${id}`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${KV_TOKEN}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ value: JSON.stringify(result), ex: 7776000 })
    });

    // Maintain sorted list of all result IDs (newest first)
    ...
    // Add new ID at front, keep last 500
